...
public class Main
{
    ...
    public static void main(String[] args) throws FileNotFoundException 
    {
        ...
        hotel.fillMembers("hotelMembers.txt");
    ...
    private static void makeReservationMenu() throws FileNotFoundException 
    {
        ...
        while(inputChk){
            System.out.println(" What is the date you want to stay? 'mm/dd/yyyy - mm/dd/yyyy'");  
            try{
                input.nextLine();
                ts = toTimeSpan(input.nextLine());
                inputChk = false;
        ...
        ArrayList<Room> availableRooms = hotel.getEmptyRooms(ts);
        ...
        availableRooms = roomOptionsTrimmer(partySize, priceRange, hotel.getEmptyRooms(ts));
        ...
        System.out.println(" Are you a member of Hotel Burger? Enter 1 for yes, or 0 for no"); 
        int in = getUserInputInt(0,1);
        if(in == 1){
            guest = findMember();
            if(guest == null){
                guest = nonMemberReservation();
                if(guest == null) returnToMainMenuPrompt();
            }
        }else{
            guest = nonMemberReservation();
            if(guest == null) returnToMainMenuPrompt();
        ...
        System.out.println(" Would you register for the member of Hotel Burger to enjoy exclusive discounts? Enter 1 for yes, or 0 for no");
        in = getUserInputInt(0,1);
        if(in == 1){
            int temp = hotel.getMembers().addMember(guest);
            System.out.println(" Registration successful! Your membership number is " + temp);
        }
        ...
        try
        {
            ...
            reservation = new Reservation(room, guest, Status.IN, partySize, ts);
            reservation.getRoom().addTimeSpan(ts);
    ...
    private static Guest nonMemberReservation(){
        while(true) {   
            System.out.println(" What is your first name? Enter 0 to back to main menu.");
            ...
            while(!inputChk){
                ...
                inputChk = phoneCheck(temp);
                if(inputChk){
                    phoneNumber = temp;
                }else{
                    System.out.println(" Not a valid phone number. Please re-enter.");
                }
            }
            // next get discount statuses
            System.out.println(" Are you active military? Enter 1 for yes, or 0 for no");        
            int in = getUserInputInt(0,1);
            boolean isMilitary = (in == 1);
    ...
    private static Guest findMember(){
        Guest guest;
        while(true){
            System.out.println(" Please enter the 9-digit membership number. Type forget to retrieve. Type 0 to return to non-member reservation." );
            String memberNum = input.next();
            String phone = "";
            if(memberNum.toLowerCase().equals("forget")){
                ...
                try{
                    return guest = hotel.getMembers().findMember(name, phone);
                }catch(Exception e){
                    System.out.println(" No member was found under this criteria, please try again.");
                }
            }else if(memberNum.equals("0")){
                return null;
            }else{
                try{
                    return guest = hotel.getMembers().findMember(memberNum);
                }catch(IllegalArgumentException e){
                    System.out.println(" Not a valid membership number, please try again.");
    ...
    private static void changeReservationMenu() throws FileNotFoundException 
    {
        ...
        TimeSpan ts = reservation.getTimeSpan();
        ...
        switch( selectionInt )
        {
            ...
            try {
                ...
                reservation.getRoom().removeTimeSpan(reservation.getTimeSpan());
                break;
            } catch (IllegalArgumentException e) {
            ...
            for (Room r : hotel.getEmptyRooms(ts))
            {
            ...
            while ( newRoom == null || !newRoom.isAvailable(ts))
            {
    ...
    {   
        ...
        System.out.println(" Please enter the date you would like to check availability (): \n");
        TimeSpan ts = toTimeSpan(input.next());
        System.out.println( " These rooms are available:");
        for (Room r: hotel.getEmptyRooms(ts))
        {
            System.out.println(r);
