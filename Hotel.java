...
public class Hotel
{
    ...
    private HotelLinkedList<Reservation> reservations;
    private HotelHashMap<String, Room> rooms;
    ...
    {     
        rooms = new HotelHashMap<String, Room>();
        reservations = new HotelLinkedList<Reservation>();
        members = new Member();
    ...
    public void fillRoomArrayList() throws FileNotFoundException
    {
        ...
        {
            HotelTree<TimeSpan> times = new HotelTree();
    ...
    public void fillReservationArrayList(String fileName) throws FileNotFoundException
    {
        ...
        while(input.hasNextLine()) 
        {
            ...
            timespan = toTimeSpan(input.next());
            ...
            reservation = new Reservation(room, guest, status, partySize, timespan);
     ...
     * @throws IllegalArgumentException if the file doesn't match expected format.
     */
    public void fillMembers(String fileName) throws FileNotFoundException
    {
        File inFile = new File(fileName);
        input = new Scanner(inFile);
        ...
        members.setMemberNumber(input.nextInt());
        while(input.hasNextLine()) 
        {       
            // We expect String first name.
            firstName = input.next(); 
    ...
    public void save() throws FileNotFoundException 
    {
        ...
        for(Reservation r : reservations) {
            ...
            output.print(g.getMemberNumber() + " ");
            output.print(r.getStatus() + " ");
            output.print(r.getTimeSpan());
        }

        output = new PrintStream(new File("hotelroomsOutput.txt"));
        output.println(name);
        output.println(address);
        output.println(phoneNumber);
        for(Room r : rooms.valueSet()){
            output.print(r.getRoomNumber() + " ");
            output.print(r.getFloor() + " ");
            output.print(r.getCapacity() + " ");
            output.print(r.getBedType() + " ");
            output.print(r.getRoomType());
            output.println(r.getTimes().toStringPre() + "\n");
        }

        output = new PrintStream(new File("hotelMembers.txt"));
        output.println(members.getMemberNumber());
        for(Guest g : members.getMembers()){
            output.print(g.getFirstName() + " ");
            output.print(g.getLastName() + " ");
            output.print(g.getPhoneNum() + " ");
            output.print(g.isMilitary() + " ");
     ...
     */
    public ArrayList<Room> getEmptyRooms(TimeSpan ts) 
    {
        ArrayList<Room> rms = new ArrayList<Room>();

        for (Room rm: rooms.valueSet()) 
        {
            if ( rm.isAvailable(ts) ) 
            {
                rms.add(rm);
            }
        }
    ...
    public ArrayList<String> getOccupiedRoomNum(TimeSpan ts) {
    ...
    public ArrayList<String> getEmptyRoomNum(TimeSpan ts) {
        ArrayList<String> rms = new ArrayList<>();
        for(Room rm: rooms.valueSet()) {
            if(!rm.isAvailable(ts)) {
                rms.add(rm.getRoomNumber());
