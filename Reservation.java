...
public class Reservation
{
     ...
     */
    public Reservation( Room r, Guest g, Status status, int partySize, TimeSpan timespan)
    {
        // call setRoom first 
    ...
    public void setStatus( Status s )
    {
        ...
        {
            if (this.getStatus() == Status.OUT || this.getStatus() == Status.IN ) 
            {
                throw new IllegalArgumentException("You can't cancel this reservation.");
            }
        ...
        if ( s == Status.IN )
        {
            if (this.getStatus() == Status.CANCELED || this.getStatus() == Status.OUT) 
            {
                throw new IllegalArgumentException("Reservation was already canceled / checked out.");
            }            
        }
        ...
        if ( s == Status.OUT ) 
        {
            if (this.getStatus() == Status.CANCELED || this.getStatus() == Status.WAITING)
            {
                throw new IllegalArgumentException("The reservation cann't check out currently.");
            }            
        }
        ...
        if ( s == Status.WAITING ) 
        {
            if (this.getStatus() == Status.CANCELED || this.getStatus() == Status.IN || this.getStatus() == Status.OUT)
            {
                throw new IllegalArgumentException("Can't set state to waiting");
            }
        }
    ...
    public void changeRoom( Room other )
    {
        ...
        this.r.removeTimeSpan(timespan);
    ...
    public String toString()
    {
        ...
        "Status: " + getStatus() + '\n' +
        r.toString() + '\n' +
        "Date of stay: " + timespan.toString() + '\n' + 
